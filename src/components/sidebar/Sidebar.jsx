...
const Sidebar = () => {
    ...
    return (
        <div className="w-2/12 bg-[#212121] pr-5 overflow-auto pb-8 sidebar">
            {[
                { title: "Main Links", links: mainLinks },
                { title: "Secondary Links", links: secondaryLinks },
                { title: "Subscription Links", links: subscriptionLinks },
                { title: "Help Links", links: helpLinks },
            ].map(({ title, links }) => (
                <ul key={title} className="flex flex-col border-b-2 border-gray-700">
                    {links.map(({ icon, name }) => (
                        <SidebarItem key={name} icon={icon} name={name} />
                    ))}
                </ul>
            ))}
            {textLinks.map((links, index) => (
            ...
            <span className="px-4 text-sm text-zinc-400">&copy; {new Date().getFullYear()} </span>
            <br />
            <p className="px-4 pt-3 text-sm text-zinc-400">
                Modified clone for project.
            </p>
        </div>
    );
};
